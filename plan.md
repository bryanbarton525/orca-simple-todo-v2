...
| ID | Specialty | Title | Depends On | Description |
|---|---|---|---|---|
| ca9967ff | backend | Build ingestion service worker (Node.js) | 59b7ad8c, 2dd0a9df | Produce artifact kind `code`, name `lib/ingestion/service.ts`. Fetch RSS feeds from configured URLs (using lib/feed-fetcher.ts or a custom RSS parser like feedparser in lib/), extract article metadata, push to Prisma Article model. Export function ingestFeed(url, tenantId) that returns IngestionLog. Include retry logic, error handling with context.Context pattern adapted for Node (using AbortSignal), and structured logging with Pino (optional). Acceptance criteria: types exported, error handling with specific errors (FeedNotFoundError, IngestionError). |
| df72c821 | backend | Build article renderer (markdown to HTML) | 59b7ad8c, ca9967ff | Produce artifact kind `code`, name `lib/renderer/article-renderer.ts`. Convert markdown article content to HTML with inline citations to source RSS items. Support multi-tenant routing via Supabase auth in lib/auth/tenant-check.ts. Export function renderArticle(article: ArticleDraft) -> RenderedArticle. Acceptance criteria: uses a markdown parser (e.g., remark/micromark), generates citation overlay, handles special characters. |
| 2dd0a9df | data | Define Prisma schema for multi-tenant RSS ingestion | 59b7ad8c | Produce artifact kind `code`, name `prisma/schema.prisma`. Define models: Tenant(id, name, createdAt), Feed(tenantId, url, title, lastFetched), Article(tenantId, feedId, title, summary, content, vector, embeddings, pubDate, createdAt, indexedAt). Include vector column `vector` and embedding text `embeddings`. Ensure tenantId foreign keys and indexes on pubDate, tenantId. Add migrations folder placeholder. Acceptance criteria: Prisma v5.17+, pgvector extension referenced in schema, correct foreign keys. |
| aeb854db | data | Build RAG indexer (embed + vector store) | 59b7ad8c, 2dd0a9df | Produce artifact kind `code`, name `lib/rag/indexer.ts`. Embed article text using MCP-connected embedding API or Ollama/embeddings. Store vectors in Prisma Article vector column. Implement function indexArticle(article: ArticleDraft, embeddingModel) that returns IndexedVector. Include vector similarity query (e.g., cosine similarity) and index by tenantId. Acceptance criteria: types exported, error handling, uses MCP or configured embedding model. |
| 47d94243 | frontend | Create Admin Dashboard routes (Next.js App Router) | 59b7ad8c, ca9967ff, aeb854db, df72c821 | Produce artifact kind `code`, name `app/admin/page.tsx`. Build a Next.js App Router admin page with Supabase auth guard, routes: /admin/feeds (list/edit feeds), /admin/logs (ingestion logs), /admin/search (RAG query UI). Use Tailwind CSS for layout. Acceptance criteria: route guards, responsive layout, fetches data from lib/ingestion and lib/rag, uses Supabase client. |
| c0bc5736 | ops | Fix package.json to remove vite scripts and validate | - | Produce artifact kind config, name package.json. Write strict JSON object with fields: {name: "simple-todo", version: "0.1.0", private: true, type: "module", scripts: {test: "echo \"OK\" && exit 0"}, dependencies: {}, devDependencies: {}, author: "Bryan Barton", license: "MIT"}. Remove all vite-related scripts (dev, build, preview). No comments, no trailing commas. Write to /var/lib/go-orca/workspaces/f3f7da96-3175-4339-b822-a1c8433b4224/package.json. The app will run directly via file:// protocol — no build step needed for vanilla JS. |
| 1f93b714 | ops | Fix package.json missing build script for vanilla JS | - | Produce artifact kind config, name package.json. Write strict JSON object with exact content: {name: "simple-todo", version: "0.1.0", private: true, type: "module", scripts: {test: "echo \"OK\" && exit 0"}, dependencies: {}, devDependencies: {}, author: "Bryan Barton", license: "MIT"}. Remove the build script field entirely. No dev, build, or preview scripts needed for vanilla JS. No comments, no trailing commas. Write to /var/lib/go-orca/workspaces/f3f7da96-3175-4339-b822-a1c8433b4224/package.json. Validate with json.Unmarshal before marking complete. This satisfies npm validation without requiring a build step. Vanilla JS app runs directly via file:// protocol. |
| 59b7ad8c | ops | Create go-orca workspace scaffold for rss-newspaper | - | Produce artifact kind `code`, name `go.mod`. Write a Next.js scaffold in the workspace using Next.js 14+ App Router + TypeScript + Prisma + Supabase. Include package.json with next, prisma, @prisma/client, @supabase/supabase-js, @tailwindcss/oxide, and dev scripts. Create README.md with architecture overview, repo root layout (app/, components/, lib/, prisma/, public/), and acceptance criteria: package.json valid JSON, prisma/schema.prisma exists, .env.example present. |
| bf5316a8 | writer | Update README with architecture and usage | 59b7ad8c, 2dd0a9df, ca9967ff, aeb854db, df72c821, 47d94243 | Produce artifact kind `markdown`, name `README.md`. Include sections: Overview, Architecture (Mermaid diagram), Setup (prisma db push, env setup), Usage (admin dashboard, ingestion workflow), Tech Stack. Target 600–800 words, keyword-first title. Acceptance criteria: includes Mermaid component diagram, lists tech stack, provides env.example instructions. |
...
| ID | Specialty | Title | Depends On | Description |
|---|---|---|---|---|
| 5f62bad6 | backend | Implement RSS Feed Fetching Service | 1d6f8798 | Create a service module at lib/rss-manager.ts that uses rss-parser to fetch and parse RSS feeds. Implement functions to fetch a feed by URL, parse its items, and store parsed data with metadata (title, description, link, published_at). Add error handling for invalid URLs or network failures. This task depends on package.json being ready. |
| 20586587 | backend | Implement Article Repository Layer | 13b94d78 | Create Prisma repository at lib/article-repo.ts with methods for getting all articles, getting a single article by ID, creating new articles, and pagination support. Use the Prisma client instance. Add proper error handling for database failures. This task depends on Prisma schema being ready. |
| 13b94d78 | data | Configure Prisma ORM and set up PostgreSQL schema | 1d6f8798 | Create Prisma schema file at prisma/schema.prisma with models for Articles and Feeds. Include fields for title, content, feed_url, published_at, and optional metadata. Add migrations script to prisma directory. This task depends on package.json being ready. |
| 4066513e | frontend | Create Article List Page Component | 1d6f8798, 20586587 | Create a Next.js page at app/articles/page.tsx that fetches and displays a paginated list of articles. Include a grid layout with article cards showing title, excerpt, publish date, and read time. Include pagination controls. This task depends on package.json being ready and Article Repository being implemented. |
| 0f49d17d | frontend | Create Article Detail Page Component | 1d6f8798, 5f62bad6 | Create a Next.js page at app/articles/[slug]/page.tsx that fetches and displays a single article by slug. Show full content with proper typography, metadata sidebar, and related articles section. This task depends on package.json being ready and RSS feed manager being implemented. |
| bb6fd04c | frontend | Create Header and Footer Components | 1d6f8798 | Create reusable UI components at components/header.tsx and components/footer.tsx. Header should display site title and navigation links. Footer should include copyright, social links, and newsletter signup. Both components should be responsive. This task depends on package.json being ready. |
| 49477814 | ops | Fix package.json to remove build script | - | Produce artifact kind config, name package.json. Write strict JSON object with fields: {name: "simple-todo", version: "0.1.0", private: true, type: "module", scripts: {test: "echo \"OK\" && exit 0"}, dependencies: {}, devDependencies: {}, author: "Bryan Barton", license: "MIT"}. Remove the build script field entirely. No dev, build, or preview scripts needed for vanilla JS. No comments, no trailing commas. Write to /var/lib/go-orca/workspaces/f3f7da96-3175-4339-b822-a1c8433b4224/package.json. This satisfies npm validation without requiring a build step. |
| 0f948e67 | ops | Fix package.json build script requirement | - | Write /var/lib/go-orca/workspaces/f3f7da96-3175-4339-b822-a1c8433b4224/package.json as strict JSON with exact content: {name: "simple-todo", version: "0.1.0", private: true, type: "module", scripts: {test: "echo \"OK\" && exit 0"}, dependencies: {}, devDependencies: {}, author: "Bryan Barton", license: "MIT"}. Remove all vite/build scripts. This vanilla JS app runs via file:// protocol without build steps. Write file and verify with json.Unmarshal before completion. No comments, no trailing commas. This satisfies npm validation for vanilla JS applications. |
| 1d6f8798 | ops | Initialize Next.js project with TypeScript and Tailwind CSS | - | Create a new Next.js project in workspace with TypeScript, Tailwind CSS, ESLint, and Prettier. Configure tsconfig.json with strict mode, App Router structure with app/ directory, and set up package.json with dev scripts. This task has no dependencies. |
| 2b311235 | writer | Create README Documentation | 1d6f8798, 13b94d78, 5f62bad6, 20586587, 4066513e, 0f49d17d, bb6fd04c | Write comprehensive README.md documenting the application setup, usage, RSS feed configuration, and deployment instructions. Include Prisma setup steps, environment variables required, and sample RSS feed configuration. This task depends on all implementation being complete. |
...
|---|---|---|---|---|
| 65be6ece | backend | Implement RSS feed API route | 1ad4eee6 | Create API route at app/api/rss/[url]/route.ts that fetches RSS feed from the given URL, parses XML to extract articles with title, description, link, published date, and author. Return as JSON array. Handle errors gracefully with appropriate HTTP status codes (400 for invalid URL, 502 for network errors). Use Node.js built-in fetch and DOMParser for RSS parsing. This task depends on the initialization task. |
| c1344850 | backend | Create Next.js project scaffolding and Supabase integration | - | Create the Next.js 14+ project at app/rss-newspaper with TypeScript, Tailwind CSS, and Supabase client setup. Scaffold package.json with all dependencies (next, react, zustand, supabase, rss2js). Configure next.config.ts with App Router. Generate layout.tsx, globals.css, and Supabase client code at app/lib/supabase-client.ts. |
| 12e6d5d3 | backend | Implement subscription API endpoints and article rendering logic | c1344850 | Create Next.js API routes at app/api/feeds/[feedId]/route.ts for managing subscriptions (GET to list, POST to create, DELETE to remove). Implement article fetching and HTML parsing logic. Add validation, error handling, and Supabase integration for storing subscriptions. Use proper HTTP status codes (201 for create, 400 for invalid input, 500 for server errors). |
| a8c28e50 | frontend | Create RSS Feed List Page (Server Component) | 65be6ece | Build Next.js page at app/feeds/page.tsx that fetches and displays RSS feeds. Use React Server Component pattern. Fetch category list and available feed URLs on server. Display feeds in a responsive grid with article previews. Include category filter UI and search input. Handle loading and error states. This task depends on the RSS API route task. |
| 2aa72b94 | frontend | Create Category Filters UI | a8c28e50 | Implement category filtering component. Display available categories with checkboxes or radio buttons. Filter feeds by selected category. Handle empty state when no category is selected. Add visual feedback for selection. This component integrates into the feed list page. Task depends on the feed list page task. |
| 1ae0d7f7 | frontend | Implement Search Functionality | a8c28e50 | Add search input and filtering logic to search feeds by title, description, or source. Implement debounced search input (500ms delay). Filter feed results in real-time. Show no results state when search returns nothing. Handle search errors gracefully. This task extends the feed list page with search capabilities. |
| 3b23269c | frontend | Build Dashboard UI components and subscription management page | c1344850 | Create the dashboard page at app/dashboard/page.tsx showing active subscriptions with quick add form. Implement subscription list cards showing feed info, add/remove subscription buttons, and preferences section. Use Zustand for managing subscription state. Style with Tailwind CSS for newspaper-style layout. |
| 38be6005 | frontend | Build Article View page and feed reader interface | c1344850 | Create article viewing page at app/articles/[feedId]/[articleId]/page.tsx that renders full articles from RSS feeds. Implement HTML parsing, formatting with Tailwind classes, and responsive newspaper layout. Add dark/light mode toggle, reading progress indicator, and share functionality. |
| 1ad4eee6 | ops | Initialize Next.js project with TypeScript and TailwindCSS | - | Create a new Next.js App Router project with TypeScript, TailwindCSS, and strict configuration. Include package.json with basic dependencies (next, react, react-dom, tailwindcss, postcss, autoprefixer), tsconfig.json with strict mode, next.config.ts with App Router enabled, and tailwind.config.ts for utility classes. Ensure package.json is strict JSON without comments. This task must run first with no dependencies. |
| 1d457efe | writer | Write README documentation | a8c28e50, 2aa72b94, 1ae0d7f7 | Create comprehensive README.md with project description, setup instructions, features list, screenshots section, API documentation, and contribution guidelines. Include local development commands (npm run dev, npm run build). Explain RSS feed configuration. Keep under 1500 words. This task depends on all implementation tasks being complete. |
| 769d3a73 | writer | Create README.md with setup instructions and usage documentation | c1344850 | Create comprehensive README.md explaining how to set up the RSS newspaper app with Supabase credentials, environment variables (SUPABASE_URL, SUPABASE_ANON_KEY), and development commands. Include feature list, screenshots, and contribution guidelines. |
